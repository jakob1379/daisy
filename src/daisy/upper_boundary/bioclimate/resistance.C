...
// 
// Copyright 2008 Per Abrahamsen, Birgitte Gjettermann, and SÃ¸ren Hansen and KU
//
...
double 
Resistance::gbu_H2O_amph (const double gbu_heat /*[m sÂ¯1]*/, const double Cl /*[]*/)
{
  const double gbu_H2O = gbu_heat * (d_H2O * Cl)/(d_heat * Cl);  
  return gbu_H2O; // [m sÂ¯1]
}
// For hypostomatous leaves (G6)
double 
Resistance::gbu_H2O_hypo (const double gbu_heat /*[m sÂ¯1]*/, const double Cl /*[]*/)
{
...
double 
Resistance::gc_fraction (const double gb_j /*[m sÂ¯1]*/, 
                         const double ra /* aerodynamic resistance (F5) [s mÂ¯1]*/)
{
...
Resistance::T_c (const double T_l_sun /* leaf sun temperature [dg C]*/, 
                 ...
                 const double LAI /*[m^2 m^-2]*/) 
{
  // Plauborg et al (2010), Equation (A6).
