...
void SurfaceSourceFluxModel::tick (const Time& time,
                                   double dt /* [h] */,
...
void SurfaceSourceFluxModel::tick_source (const Time& time, Treelog& msg)
{
  ...
  // we have separated the object model from the actual models, so it is not possible atm.
  // TREELOG_MODEL (msg);
...
SurfaceSourceFluxModel::SurfaceSourceFluxModel (const Units& units, bool interpolate, Source *source)
    : SurfaceSimpleModel (),
      units (units),
...
SurfaceSourceFluxModel::~SurfaceSourceFluxModel ()
{ }
